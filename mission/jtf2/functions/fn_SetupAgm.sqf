...
if (isClass (configFile >> "CfgPatches" >> "AGM_Medical") && isServer) then
{
	...
	diag_log format["Applied AGM Medical options. (%1, %2, %3, %4)", _missionParamInvincibleWhileDead, _missionParamPreventInstantDeath, _missionParamDeathTimer, _missionParameterAllowAutomaticWakeup];
}
else
{
...
if (isClass (configFile >> "CfgPatches" >> "AGM_Map")) then
{
	...
	{
		diag_log "Not enabling AGM map options due to mission parameters.";
	};
}
else
{
