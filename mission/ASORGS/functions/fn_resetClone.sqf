...
{ ASORGS_Clone removeMagazine _x; } forEach (magazines ASORGS_Clone);
{ ASORGS_Clone removeItem _x; } forEach (items ASORGS_Clone);
{ ASORGS_Clone unassignItem _x; ASORGS_Clone removeItem _x; } forEach (assignedItems ASORGS_Clone);

if(uniform ASORGS_Player != "") then {ASORGS_Clone forceAddUniform uniform ASORGS_Player; };
if(headgear ASORGS_Player != "") then {ASORGS_Clone addHeadgear headgear ASORGS_Player;};
if(vest ASORGS_Player != "") then {ASORGS_Clone addVest vest ASORGS_Player;};
if(backpack ASORGS_Player != "") then {ASORGS_Clone addBackpack backpack ASORGS_Player;};
if(goggles ASORGS_Player != "") then {ASORGS_Clone addGoggles goggles ASORGS_Player;};
{ ASORGS_Clone addMagazine _x; } forEach (magazines ASORGS_Player);
{ ASORGS_Clone addItem _x; ASORGS_Clone assignItem _x; } forEach assignedItems ASORGS_Player;
ASORGS_Clone setFace face ASORGS_Player;

_insignia = ASORGS_Player call BIS_fnc_getUnitInsignia;

[ASORGS_Clone, _insignia] call ASORGS_fnc_setUnitInsignia;
_logo = ASORGS_BackgroundLogo;
if(ASORGS_UnitInsigniaAsBackground && (_insignia != "")) then {
	_logo = getText (configFile >> "CfgUnitInsignia" >> _insignia >> "texture");
};
ASORGS_Background setObjectTexture [0, _logo];

if(primaryWeapon ASORGS_Player != "") then {
	ASORGS_Clone addWeapon primaryWeapon ASORGS_Player;
	removeAllPrimaryWeaponItems ASORGS_Clone;
	{
		if(_x != "") then {ASORGS_Clone addPrimaryWeaponItem _x;};
	}forEach( primaryWeaponItems ASORGS_Player);
};
if(secondaryWeapon ASORGS_Player != "") then {
	ASORGS_Clone addWeapon secondaryWeapon ASORGS_Player;
	//removeAllSecondaryWeaponItems ASORGS_Clone; command doesn't exist :(
	{
		if(_x != "") then {ASORGS_Clone addSecondaryWeaponItem _x;};
	} forEach(secondaryWeaponItems ASORGS_Player);
};
if(handgunWeapon ASORGS_Player != "") then {
	ASORGS_Clone addWeapon handgunWeapon ASORGS_Player;
	removeAllHandgunItems ASORGS_Clone;
	{
		if(_x != "") then {ASORGS_Clone addHandgunItem _x;};
	} forEach(handgunItems ASORGS_Player);
};*/
if(_changed) then {
	[] spawn {
	//	sleep 0.3;
