...
_binocarray = (ASORGS_DB select DB_Binoculars);
_helmarray = (ASORGS_DB select DB_Headgear);
{   _class = _x;
	...
	if (_addItem) then {
		_binocdetails = [_class,_binocarray] call ASORGS_fnc_GetDetails;
		if(count (_binocdetails) < 1) then {	//not binoculars
			if(count ([_class, _helmarray] call ASORGS_fnc_GetDetails) < 1) then { //not headgear
