...
_ignore = [];
switch(_firstIDC) do {
	case ASORGS_primaryAmmo_label: {
		_db=DB_Magazines;
		_weapondb = DB_Rifles;
		_weaponCombo = ASORGS_getControl(ASORGS_Main_Display, _weaponComboIDC);
		_weapon = _weaponCombo lbValue (lbCurSel _weaponCombo);
		if(_weapon >  -1) then {
		_weaponDetails = (ASORGS_DB select _weapondb select _weapon);
		{_list = _list + [(ASORGS_DB select _db select _x)];} forEach (_weaponDetails select DBF_Magazines);
		};
		
	};
	case ASORGS_launcherAmmo_label: {
		_db=DB_Magazines;
		_weapondb = DB_Launchers;
		_weaponCombo = ASORGS_getControl(ASORGS_Main_Display, _weaponComboIDC);
		_weapon = _weaponCombo lbValue (lbCurSel _weaponCombo);
		if(_weapon > -1) then {
		_weaponDetails = (ASORGS_DB select _weapondb select _weapon);
	...
	case ASORGS_handgunAmmo_label: {
		_db=DB_Magazines;
		_weapondb = DB_Handguns;
		_weaponCombo = ASORGS_getControl(ASORGS_Main_Display, _weaponComboIDC);
		_weapon = _weaponCombo lbValue (lbCurSel _weaponCombo);
		if(_weapon >  -1) then {
		_weaponDetails = (ASORGS_DB select _weapondb select _weapon);
	...
	case ASORGS_extraammo_label: {
		_db = DB_Magazines;
		_list = (ASORGS_DB select _db);
		{
			_thiscontrol = _x select 0;
			_thisdb = _x select 1;
			_thisindex = (_thiscontrol lbValue (lbCurSel _thiscontrol));
			if(_thisindex > -1) then {
			  _ignore = _ignore + (ASORGS_DB select _thisdb select _thisindex select DBF_Magazines); 
			};
		} forEach [[ASORGS_getControl(ASORGS_Main_Display, ASORGS_primary_combo), DB_Rifles], 
		[ASORGS_getControl(ASORGS_Main_Display, ASORGS_launcher_combo), DB_Launchers], 
		[ASORGS_getControl(ASORGS_Main_Display, ASORGS_handgun_combo), DB_Handguns]];
	};
};
...
switch(_type) do {
	case "mags": {
		{
			...
			if(_cacheIndex < 0) then {
				_details  = [_magClass, _list] call ASORGS_fnc_getDetails;
				if((count _details) > 0) then {
					_cacheIndex = count _magCacheNames;
					_magCacheNames set [_cacheIndex, _magClass];
			...
			if(_cacheIndex > -1) then {
				switch(_firstIDC) do {
					...
					case ASORGS_extraammo_label: {
						...
						if( (_details select DBF_DB) == DB_Magazines) then {
							_magIndex = _details select DBF_Index;
							if( !(_magIndex in _ignore)) then {
								_playerHas = _playerHas + [_cacheIndex];
							};
						};
					};
					default {
						if( ((_details select DBF_DB) == DB_Magazines) && (_weapon > -1) ) then {
							//can't avoid looking up from string						
							_magIndex = _details select DBF_Index;
							if(_magIndex in (_weaponDetails select DBF_Magazines)) then {
								_playerHas = _playerHas + [_cacheIndex];
							};
						};
					};
	...
	case "medical": {
		_comboCount = 20;
		{
			...
			if(_cacheIndex < 0) then {
				_details  = [_itemClass, ASORGS_DB select _db ] call ASORGS_fnc_getDetails;	
				if((count _details) > 0) then {
					_cacheIndex = count _magCacheNames;
