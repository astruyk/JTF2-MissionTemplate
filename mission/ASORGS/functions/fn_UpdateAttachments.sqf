...
if(!isNil "_scopeControl") then {	
	...
	if((count _weaponDetails) > 0) then {
		...
		{
				_details = [_x,ASORGS_DB select DB_Scopes] call ASORGS_fnc_getDetails;
				if((count _details > 0) && !((_details select 0) in ASORGS_RuntimeBlacklist)) then
				{
...
if(!isNil "_railControl") then {
	...
	if((count _weaponDetails) > 0) then {
		...
		{
			_details = [_x,ASORGS_DB select DB_Rail] call ASORGS_fnc_getDetails;
...
if(!isNil "_suppControl") then {
	...
	if((count _weaponDetails) > 0) then {
		...
		{
			_details = [_x,ASORGS_DB select DB_Suppressors] call ASORGS_fnc_getDetails;
			if((count _details > 0) && !((_details select 0) in ASORGS_RuntimeBlacklist)) then
			{
